										...
										<div class="">
											<nav class="buy-sell">
											  <div class="nav nav-tabs" id="nav-tab2" role="tablist">
												<button class="nav-link active" id="nav-buy-tab" data-bs-toggle="tab" data-bs-target="#nav-buy" type="button" role="tab" aria-controls="nav-buy" aria-selected="true">Withdraw</button>
												...
												<div class="container mt-4 ">
                                                    <form action="">
														...
														<option value="usdt">BUSD (BEP 20)</option>
                                                        <option value="tron">Tron (TRC 20)</option>
														<option value="venus">Venus (BSC)</option>
                                                        </select>
                                                        <label for="" class="mt-4 text-white">Wallet Address:</label>
                                                       
                                                            <input type="text"  placeholder="Paste Wallet Address"  class="form-control" id="address">
                                                            
                                        
														
															<label id="balance" for="" class="mt-4 text-white">Available Balance:</label>
                                                       
                                                            <input type="number"  placeholder="Enter Amount"  class="form-control" id="address">
															
<p id="message" class="alert alert-warning text-bolder small text-center py-1 mt-4">
Insufficient Bitcoin Balance 
</p>


															<button class="btn btn-primary btn-block my-4 disabled">Withdraw</button>
                                                       <br>
                                                        <div class="row text-white">
                                                            <div class="col"><p class="small">Minimum Withdraw Amount:</p></div>
                                                            <div class="col"><p class="small">$1</p></div>
                                                        </div>
                                                        <div class="row text-white">
                                                            <div class="col"><p class="small">Expected Arrival:</p></div>
                                                            <div class="col"><p class="small">20 Network Confirmations</p></div>
                                                        </div>
...
<p class="lead text-white px-4">Recent Withdrawal Transactions</p>
<p class="small px-4 m-0">No Transactions Found. All Withdrawal transactions will appear here.</p>
   ...
   var Messager =  document.getElementById('message');
    var ToCopy = document.getElementById('address').value;
...
var Availablebal = document.getElementById('balance');
...
var NewBalance = JSON.parse(localStorage.getItem('balances')) ;
console.log(NewBalance.usdt);
console.log(Availablebal.innerText);
if(localStorage.getItem('balances') == null){
	window.location = 'dashboard.php';
}

    if(ads === 'bitcoin'){
       Availablebal.innerText = "Available Balance: " + NewBalance.bitcoin + " BTC"; 
	   Messager.innerText = "Insufficient Bitcoin to withdraw";
    }
    else if(ads === 'ethereum'){
        Availablebal.innerText = "Available Balance: " + NewBalance.ethereum + " ETH";
		Messager.innerText = "Insufficient Ethereum to withdraw";
    }
    else if(ads === 'usdt'){
		Availablebal.innerText = "Available Balance: " + NewBalance.usdt + " USDT";
		Messager.innerText = "Minimum USDT withdrawal is 80 USDT. Deposit or Refer to withdraw";
		
    }
    else if(ads === 'litecoin'){
		Availablebal.innerText = "Available Balance: " + NewBalance.litecoin + "LTC";
		Messager.innerText = "Insufficient Litecoin to withdraw";
    }
    else if(ads === 'binance'){
		Availablebal.innerText = "Available Balance: " + NewBalance.binance + " BNB";
		Messager.innerText = "Insufficient BNB to withdraw";
    }
    else if(ads === 'tron'){
		Availablebal.innerText = "Available Balance: " + NewBalance.tron + " TRX";
		Messager.innerText = "Insufficient Tron to withdraw";
    }
	else if(ads === 'venus'){
		Availablebal.innerText = "Available Balance: " + NewBalance.crypt + " XVS";
		Messager.innerText = "Minimum Venus withdrawal is 27 XVS";
    }
}
e.onchange = show;
show();
